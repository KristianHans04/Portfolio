...
import { buildAcknowledgment, buildOwnerNotification } from "../lib/email/templates";
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    ...
    return jsonResponse(
      {
        ok: false,
        error:
          "Server configuration incomplete. Set TURNSTILE_SECRET_KEY, CONTACT_TO_EMAIL, and CONTACT_FROM_EMAIL.",
      },
      500,
    );
  }
  ...
  try {
    ...
    return jsonResponse(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Email provider configuration error.",
      },
      500,
    );
  }
